...
bool HelpWindow::GoTo0(const String& link)
{
	...
	if(WhenMatchLabel) {
		...
		return view.GotoLabel([CAP_BY_VAL_X_THIS](const WString& data) { return WhenMatchLabel(data, lw); }, true, true) || lnk;
	}
...
HelpWindow::HelpWindow()
{
	...
	Title(t_("Help"));
	BackPaint();
	view.WhenLink = [CAP_BY_VAL_X_THIS](const String& h) { GoTo(h); };
	AddFrame(toolbar);
	...
	tree.BackPaint();
	view.BackPaintHint();
	view.WhenMouseWheel = [CAP_BY_VAL_X_THIS] (int zdelta, dword keyflags) {
		if(keyflags & K_CTRL) {
